...
import com.osanda.ocr.camera.CameraManager;
import com.osanda.ocr.camera.ShutterButton;
import com.osanda.ocr.language.LanguageCodeHelper;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    //Toast toast = Toast.makeText(this, ocrResult.getText(), Toast.LENGTH_LONG);
    //toast.show();

    super.onResume();
    Intent intent = new Intent(getBaseContext(), ShowResult.class);
    intent.putExtra("resultText", ocrResult.getText());
    startActivity(intent);
  ...
  private boolean checkFirstLaunch() {
    try {
      ...
      if (currentVersion > lastVersion) {
        ...
        String page = lastVersion == 0 ? HelpActivity.ABOUT_PAGE : HelpActivity.ABOUT_PAGE;
